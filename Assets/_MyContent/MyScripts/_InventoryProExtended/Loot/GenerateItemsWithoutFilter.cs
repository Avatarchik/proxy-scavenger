...
[System.Serializable]
public partial class GenerateItemsWithoutFilter : IItemGenerator {

	public InventoryItemGeneratorItem[] items { get; set; }

	public InventoryItemBase[] result { get; set; }

	public InventoryItemGeneratorItem[] filterGroups = new InventoryItemGeneratorItem[0];

	protected static System.Random randomGen;


	public GenerateItemsWithoutFilter(InventoryItemGeneratorItem[] filterGroups)
	{
		RandomizeSeed();
		this.filterGroups = filterGroups;
	}

	public InventoryItemBase[] GetItems(){
		InventoryItemBase[] i = new InventoryItemBase[items.Length];
	...
	public InventoryItemBase[] Generate(int minAmount, int maxAmount, bool createInstances = false)
	{
		...
		foreach (var filterGroup in filterGroups)
		{
			...
			var l = new List<InventoryItemBase>(ii);

			foreach (int i in Enumerable.Range(0, items.Count()).OrderBy(x => randomGen.Next()))
			{
				if (Random.value > filterGroup.chanceFactor)
					continue;
				/*
